...
<img src="https://github.com/jacenam/WIL-archive/assets/92138751/8037533b-49a3-4d3c-8ad7-2d391f59cad8" width="100%">

전역 코드에서 `this`는 전역 객체를 가리키므로 전역 환경 레코드의 `[[GlobalThisValue]]` 내부 슬롯에는 전역 객체 `window`가 바인딩되어 전역 객체를 가리키는 참조값을 포함하고 있다

따라서 [전역 코드에서 `this`를 참조했을 때]() 전역 환경 레코드의 `[[GlobalThisValue]]` 내부 슬롯에 바인딩되어 있는 전역 객체 `window`를 반환하는 것이다
