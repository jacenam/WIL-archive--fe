...
아래 예제를 살펴보면 JS 엔진은 먼저 런타임 이전에 전역 코드를 평가하여 전역 실행 컨텍스트를 실행한다. 그리고 런타임에는 전역 코드를 실행한다. 그러다가 함수 호출문에 도달하면 함수가 호출되어 함수 코드를 평가하여 함수 실행 컨텍스트를 생성한다. 이런식으로 먼저 스택에 쌓인 실행 컨텍스트 이후에 다른 실행 컨텍스트가 쌓이게되면, 나중에 쌓인 실행 컨텍스트부터 처리하는 구조라고 이해하면 된다
